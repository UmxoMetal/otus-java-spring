...
public class AuthorDaoImpl implements AuthorDao {
    ...
    @Override
    public List<BookToAuthorDto> getAllAuthorsWithBooId() {
        return namedParameterJdbcOperations.query("""
                        SELECT bta.BOO_ID, auth.AUT_ID, auth.AUTHOR_FIO 
                        FROM AUTHOR auth 
                        JOIN BOOK_TO_AUTHOR bta
                        ON bta.AUT_ID = auth.AUT_ID
                        """,
                (rs, rowNum) -> BookToAuthorDto
                        .builder()
